...
function createJob(inputPath, overrides = {}) {
	...
	return {
		...
		output: {
			...
			multiMono: {
				indexBase: Number(overrides.multiMonoIndexBase) === 0 ? 0 : 1,
				placement: overrides.multiMonoPlacement || 'flat',
			},
		},
		reports: {
...
function normalizeJob(rawJob, index, defaults) {
	...
	return {
		...
		output: {
			...
			multiMono: {
				indexBase: output.multiMono && output.multiMono.indexBase !== undefined
					? Number(output.multiMono.indexBase)
					: 1,
				placement: output.multiMono && output.multiMono.placement
					? output.multiMono.placement
					: 'flat',
			},
		},
		reports: {
...
function validateManifest(manifest, options = {}) {
	...
	for (const [index, job] of manifest.jobs.entries()) {
		...
		if (![0, 1].includes(job.output.multiMono.indexBase)) {
			errors.push(`${prefix}.output.multiMono.indexBase must be 0 or 1`);
		}
		if (!['flat', 'folder'].includes(job.output.multiMono.placement)) {
			errors.push(`${prefix}.output.multiMono.placement must be "flat" or "folder"`);
		}
